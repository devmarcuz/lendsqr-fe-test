...
const Login = ({ totalPosts, setTotalPosts }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!email || !password) {
      setErrorMessage("All fields must be filled"); // Set showError to true to trigger the error message
    } else if (password.length < 3) {
      setErrorMessage("Password lenght must be greater than 3");
    } else {
